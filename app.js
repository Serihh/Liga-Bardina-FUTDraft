...
function normalizeCardId(id){if(!id)return id;const s=String(id).trim();if(LIGA_BARDINA_CARDS.some(c=>c.id===s))return s;const m=s.match(/^(portero|defensa|medio|delantero)(\d+)S$/i);if(m){const esp=m[1]+m[2]+'ESP';if(LIGA_BARDINA_CARDS.some(c=>c.id===esp))return esp}return s}
function cardById(id){const normalized=normalizeCardId(id);return LIGA_BARDINA_CARDS.find(c=>c.id===normalized)}
...
"delantero29ESP":"Gareth Bale",
"delantero30ESP":"Cristiano Ronaldo",
"delantero31ESP":"Neymar Jr",
"delantero32ESP":"Pelé",
"portero13":"Henry Dículo",
"portero14":"Keylor Papas",
"defensa45":"Solomeo Paredes",
"defensa46":"Esteban Dido",
"defensa47":"Jack Hallate",
"defensa48":"Alan Brito",
"defensa49":"Sergio Gramos",
"defensa50":"Roberto Calorías",
"defensa51":"Gordi Alba",
"medio48":"Andrés Tresado",
"medio49":"Elson Brero",
"medio50":"Elvio Lento",
"medio51":"Sergio Buffets",
"medio52":"N'Gordo Kanté",
"medio53":"Andrés Ingesta",
"medio54":"Kevin de Brownie",
"delantero33":"Al-Bahad Mahmad",
"delantero34":"Casimiro Lateta",
"delantero35":"Sevino Sobretti",
"delantero36":"Delicious Júnior",
"delantero37":"Kylian Mempaché",
"delantero38":"Mohamed Salami",
"delantero35ESP":"Sevino Sobretti",
};
...
function nav(){const p=state.profile||DEFAULT_PROFILE;const shield=(CLUB_SHIELDS.find(x=>x.name===p.shield)||CLUB_SHIELDS[0]);const packCount=Array.isArray(state.packs)?state.packs.length:0;return `<header class="topbar"><div class="brand" onclick="go('home')" style="cursor:pointer">${assetImg(shield.path,'brand-crest','')}<div><h1>${esc(p.teamName)}</h1><span>${esc(p.username)}</span></div></div><nav class="nav"><button class="${state.view==='home'?'active':''}" onclick="go('home')">Draft</button><button class="${state.view==='packs'?'active':''}" onclick="go('packs')">Sobres${packCount?` <span class="nav-count">${packCount}</span>`:''}</button><button class="${state.view==='collection'?'active':''}" onclick="go('collection')">Colección</button><button class="${state.view==='squad'?'active':''}" onclick="go('squad')">Equipo</button><button class="${state.view==='profile'?'active':''}" onclick="go('profile')">Perfil</button></nav></header>`}
function render(){const isDraft=state.view==='draft';app.innerHTML=`<div class="app ${isDraft?'draft-app':''}">${isDraft?'':nav()}<main class="shell ${isDraft?'draft-shell':''} ${state.view==='squad'?'squad-shell':''}">${state.view==='home'?renderHome():state.view==='draft'?renderDraft():state.view==='tournament'?renderTournament():state.view==='profile'?renderProfile():state.view==='summary'?renderSummary():state.view==='packs'?renderPacks():state.view==='collection'?renderCollection():state.view==='squad'?renderSquad():renderHome()}</main><div id="modalRoot"></div><div class="toast"></div></div>`;if(state.packConfirm!==null)renderPackConfirm()}
...
function describeMatchEvent(ev,d){
 if(ev.type==='goal') return ev.team==='home'?`⚽ GOL · ${esc(ev.name)} ha marcado`:'⚽ GOL · El rival ha marcado';
 ...
 return ev.team==='home'?`❌ FALLO · ${esc(ev.name)} ha fallado una ocasión`:'❌ FALLO · El rival ha fallado una ocasión';
}
...
const SPECIAL_COLLECTION_ORDER=['portero1ESP','defensa4ESP','defensa8ESP','defensa29ESP','defensa33ESP','medio3ESP','medio22ESP','medio26ESP','delantero2ESP','delantero35ESP','delantero4ESP','portero12ESP','defensa44ESP','medio44ESP','delantero25ESP','delantero26ESP','delantero27ESP','delantero28ESP','medio45ESP','medio46ESP','medio47ESP','delantero29ESP','delantero30ESP','delantero31ESP','delantero32ESP','portero4ESP','defensa3ESP','medio19ESP','delantero11ESP','delantero6ESP','delantero1ESP'];
const SPECIAL_COLLECTION_RANK=new Map(SPECIAL_COLLECTION_ORDER.map((id,i)=>[id,i]));
function specialCollectionRank(c){return SPECIAL_COLLECTION_RANK.has(c.id)?SPECIAL_COLLECTION_RANK.get(c.id):SPECIAL_COLLECTION_ORDER.length+LIGA_BARDINA_CARDS.indexOf(c)}
...
function openCollectionCard(id){
  ...
  if(!root)return;
  root.innerHTML=`<div class="collection-card-lightbox" onclick="closeCollectionCard()"><div class="collection-card-lightbox-inner" onclick="event.stopPropagation()"><div class="collection-card-lightbox-card ${isOwned?'owned':'locked'}">${assetImg(c.path,'','')}</div></div></div>`;
  requestAnimationFrame(()=>root.querySelector('.collection-card-lightbox')?.classList.add('show'));
}
function closeCollectionCard(){
  const root=document.getElementById('modalRoot');
  if(!root)return;
  const overlay=root.querySelector('.collection-card-lightbox');
  if(!overlay){root.innerHTML='';return;}
  overlay.classList.remove('show');
  setTimeout(()=>{if(root.querySelector('.collection-card-lightbox')===overlay)root.innerHTML='';},180);
}
function collectionHtml(filter='todos'){const owned=new Set(state.collection||[]);const fresh=new Set(state.newCards||[]);let cards=LIGA_BARDINA_CARDS.filter(c=>filter==='todos'||(filter==='especial'&&c.special)||(filter==='normal'&&!c.special)||c.position===filter);const order=['portero','defensa','medio','delantero'];const normalCollectionRank=c=>{const n=Number((c.id.match(/\d+$/)||['0'])[0]);const inserts={portero:{13:5.5,14:7.5},defensa:{45:19.5,46:19.6,47:19.7,48:19.8,49:27.5,50:27.6,51:27.7},medio:{48:20.5,49:20.6,50:20.7,51:27.5,52:27.6,53:27.7,54:27.8},delantero:{33:11.5,34:11.6,35:11.7,36:16.5,37:16.6,38:16.7}};return inserts[c.position]?.[n]??n;};const rank=c=>order.indexOf(c.position);if(filter==='todos'){cards=[...cards.filter(c=>!c.special).sort((a,b)=>rank(a)-rank(b)||normalCollectionRank(a)-normalCollectionRank(b)),...cards.filter(c=>c.special).sort((a,b)=>specialCollectionRank(a)-specialCollectionRank(b))]}else if(filter==='especial'){cards.sort((a,b)=>specialCollectionRank(a)-specialCollectionRank(b))}
return cards.length?cards.map(c=>{const isOwned=owned.has(c.id);const isNew=isOwned&&fresh.has(c.id);return `<button type="button" class="collection-card image-only ${isOwned?'owned':'locked'} ${isNew?'is-new':''}" onclick="openCollectionCard('${c.id}')">${isNew?'<span class="collection-new-badge">NEW</span>':''}${assetImg(c.path,'','')}</button>`}).join(''):`<div class="empty">No hay cartas en este filtro.</div>`}
function resetCollection(){if(!confirm('¿Seguro que quieres resetear la colección? Perderás todas las cartas conseguidas y tendrás que volver a conseguirlas. También se vaciará tu Equipo.'))return;state.collection=[];state.newCards=[];state.squad={formation:'4-3-3',slots:Array(11).fill(null)};save();render();showToast('Colección y Equipo reseteados');}
...
function renderSquad(){
  if(!state.squad)state.squad={formation:'4-3-3',slots:Array(11).fill(null)};
  const f=state.squad.formation;
  const cards=state.squad.slots.filter(Boolean).map(id=>cardById(id)).filter(Boolean);
  const rating=cards.length?Math.round(cards.reduce((a,c)=>a+c.rating,0)/cards.length):'—';
  const special=cards.filter(c=>c.special).length;
  const me=state.profile||DEFAULT_PROFILE;
  const shield=CLUB_SHIELDS.find(x=>x.name===me.shield)||CLUB_SHIELDS[0];
  return `<section class="squad-fullscreen"><div class="squad-field"><div class="squad-pitch"><div class="pitch squad-pitch-inner">${FORMATIONS[f].slots.map((s,i)=>squadSlotHtml(s,i,state.squad.slots[i])).join('')}</div></div></div><div class="squad-hud"><div class="squad-actions"><button class="draft-exit" onclick="clearSquad()">Vaciar equipo</button></div></div><div class="squad-stats"><span>JUGADORES <b>${cards.length}/11</b></span><span>MEDIA <b>${rating}</b></span><span>ESPECIALES <b>${special}</b></span></div><aside class="squad-formation-panel"><div class="squad-formation-kicker">FORMACIÓN</div><div class="squad-formation-buttons">${Object.keys(FORMATIONS).map(x=>`<button class="squad-formation-btn ${x===f?'active':''}" onclick="setSquadFormation('${x}')">${x}</button>`).join('')}</div></aside></section>`
}
function setSquadFormation(f){
  if(!FORMATIONS[f])return;
  if(state.squad.slots.some(Boolean)&&state.squad.formation!==f){
    if(!confirm('Cambiar de formación vaciará las posiciones del Equipo. ¿Continuar?'))return;
  }
  state.squad.formation=f;
  state.squad.slots=Array(11).fill(null);
  save();render();
}
function squadRating(){const cards=state.squad.slots.filter(Boolean).map(id=>cardById(id)).filter(Boolean);return cards.length?Math.round(cards.reduce((a,c)=>a+c.rating,0)/cards.length):'—'}
function squadSlotHtml(s,i,id){return `<div class="slot squad-slot" style="left:${s[2]}%;top:${s[3]}%" onclick="openSquadModal(${i})">${id?assetImg(cardById(id).path,'player-card',''):assetImg('imagenes mejor calidad/assets/huecojugador.png','placeholder','')}</div>`}
function openSquadModal(i){
  const slot=FORMATIONS[state.squad.formation].slots[i];
  const used=new Set(state.squad.slots.filter((id,j)=>id&&j!==i).map(id=>cardById(id)?.baseId).filter(Boolean));
  let cards=(state.collection||[]).map(id=>cardById(id)).filter(c=>c&&c.position===slot[1]&&!used.has(c.baseId));
  cards=cards.sort((a,b)=>b.rating-a.rating||Number(b.special)-Number(a.special));
  const root=document.getElementById('modalRoot');
  if(!root)return;
  root.innerHTML=`<div class="pick-modal open"><div class="modal squad-modal"><div class="modal-head"><div><div class="eyebrow">Equipo · ${POS_NAMES[slot[1]]}</div><h3>Elige tu ${POS_NAMES[slot[1]].toLowerCase()}</h3><p class="modal-subtitle">Solo aparecen los ${POS_NAMES[slot[1]].toLowerCase()} que has conseguido en tu colección.</p></div><button class="btn secondary" onclick="closeModal()">Cerrar</button></div>${cards.length?`<div class="choices squad-choices">${cards.map(c=>`<button class="choice" onclick="setSquadCard(${i},'${c.id}')">${assetImg(c.path,'','')}</button>`).join('')}</div>`:`<div class="empty">Todavía no tienes ${POS_NAMES[slot[1]].toLowerCase()} en tu colección. Abre sobres para conseguir cartas.</div>`}</div></div>`
}
function setSquadCard(i,id){
  const c=cardById(id);
  if(!c||!state.collection.includes(id))return;
  const duplicate=state.squad.slots.some((x,j)=>j!==i&&x&&cardById(x)?.baseId===c.baseId);
  if(duplicate){showToast('Ese jugador ya está en tu Equipo.');return;}
  state.squad.slots[i]=id;save();closeModal();render();
}
function clearSquad(){state.squad.slots=Array(11).fill(null);save();render();}

render();
window.go=go;window.startDraft=startDraft;window.resumeDraft=resumeDraft;window.chooseCaptain=chooseCaptain;window.handleSlotClick=handleSlotClick;window.closeModal=closeModal;window.pickCard=pickCard;window.resetDraft=resetDraft;window.beginTournament=beginTournament;window.simulateMatch=simulateMatch;window.continueTournament=continueTournament;window.saveProfile=saveProfile;window.selectShield=selectShield;window.confirmPack=confirmPack;window.cancelPackConfirm=cancelPackConfirm;window.acceptPackConfirm=acceptPackConfirm;window.closePackOpening=closePackOpening;window.setSquadFormation=setSquadFormation;window.openSquadModal=openSquadModal;window.setSquadCard=setSquadCard;window.clearSquad=clearSquad;window.openCollectionCard=openCollectionCard;window.closeCollectionCard=closeCollectionCard;
